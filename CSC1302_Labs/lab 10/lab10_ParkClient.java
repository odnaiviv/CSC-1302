...
public class lab10_ParkClient {
    public static void main(String[] args){
        ...
        int tempPark = 0;
        //loop to find the park with the largest length
        for (int i = 0; i < ParkArray.length; i++) {
            //finding the largest park length value and replaces the index with the largest park length value (i)
            if (ParkArray[i].ParkLengthGetter() > ParkArray[tempPark].ParkLengthGetter()) {
                tempPark = i;
            }
        }

        //initializing largePark by putting the temporary park value i into park array
        lab10_Park largePark = ParkArray[tempPark];
        //printing results of the largest park from all parks
